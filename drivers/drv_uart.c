...
static void wm600_uart_reg_init(TLS_UART_REGS_T *UARTx)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1);
    ...
    /* disable interrupt */
        ...
        tls_reg_write32(HR_UART0_FIFO_CTRL, (0x01 << 2) | (0x02 << 4));
        /* enable rx timeout, disable rx dma, disable tx dma */
        tls_reg_write32(HR_UART0_DMA_CTRL, (8 << 3) | (1 << 2));
    #endif
...
static void wm600_uart_gpio_config(TLS_UART_REGS_T *UARTx)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1);
...
static void wm600_uart_irq_config(TLS_UART_REGS_T *UARTx)
{
...
static int wm600_uart_baudrate_set(TLS_UART_REGS_T *UARTx, u32 baudrate)
{
    u32 value;
    u32 apbclk;
    tls_sys_clk sysclk;
...
static int wm600_uart_parity_set(TLS_UART_REGS_T *UARTx, TLS_UART_PMODE_T paritytype)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1 || UARTx == WM600_UART2);
...
static int wm600_uart_stopbits_set(TLS_UART_REGS_T *UARTx, TLS_UART_STOPBITS_T stopbits)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1 || UARTx == WM600_UART2);
...
static int wm600_uart_databits_set(TLS_UART_REGS_T *UARTx, TLS_UART_CHSIZE_T charlength)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1 || UARTx == WM600_UART2);
...
static int wm600_uart_flow_ctrl_set(TLS_UART_REGS_T *UARTx, TLS_UART_FLOW_CTRL_MODE_T flow_ctrl)
{
    RT_ASSERT(UARTx == WM600_UART0 || UARTx == WM600_UART1 || UARTx == WM600_UART2);
...
static rt_err_t drv_uart_configure (struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    wm600_uart_baudrate_set((TLS_UART_REGS_T *)uart->uart_device, baud_rate);
    ...
    wm600_uart_parity_set((TLS_UART_REGS_T *)uart->uart_device, parity);
    ...
    wm600_uart_stopbits_set((TLS_UART_REGS_T *)uart->uart_device, stop_bits);
    ...
    wm600_uart_databits_set((TLS_UART_REGS_T *)uart->uart_device, data_bits);

    wm600_uart_flow_ctrl_set((TLS_UART_REGS_T *)uart->uart_device, TLS_UART_FLOW_CTRL_NONE);
...
static int drv_uart_getc (struct rt_serial_device *serial)
{
    ...
    ch = -1;
    if(((uart->uart_device->UR_FIFOS & UFS_RX_FIFO_CNT_MASK) >> 6)  & 0x3F)
...
void rt_hw_console_output(const char *str)
{
    while(*str)
    {
        ...
        while(tls_reg_read32(HR_UART0_FIFO_STATUS) & 0x3F);
        tls_reg_write32(HR_UART0_TX_WIN, (char)*str ++);
    }
}
...
int rt_hw_uart_init(void)
{
...
#ifdef RT_USING_UART0
    {
        ...
        /* Init UART Hardware */
        wm600_uart_gpio_config((TLS_UART_REGS_T *)uart->uart_device);
        wm600_uart_reg_init((TLS_UART_REGS_T *)uart->uart_device);
        wm600_uart_irq_config((TLS_UART_REGS_T *)uart->uart_device);
