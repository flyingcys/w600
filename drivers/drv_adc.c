...
rt_err_t wm_adc_enabled(struct rt_adc_device *device, rt_uint32_t channel, rt_bool_t enabled)
{
...
rt_err_t wm_adc_convert(struct rt_adc_device *device, rt_uint32_t channel, rt_uint32_t *value)
{
    if(channel < 1 || channel > 8)
        return RT_ERROR;
    ...
    *value = adc_get_inputVolt(channel - 1) ;
    ...
    /* inputVolt = (*value-8192.0)/8192*2.25/1.2 + 1.584 */
...
rt_err_t wm_cpu_temp_enabled(struct rt_adc_device *device, rt_uint32_t channel, rt_bool_t enabled)
{
    if(enabled == RT_FALSE)
    {
        tls_adc_stop(0);
...
rt_err_t wm_cpu_temp_convert(struct rt_adc_device *device, rt_uint32_t channel, rt_uint32_t *value)
{
...
INIT_DEVICE_EXPORT(wm_adc_init);
#endif
